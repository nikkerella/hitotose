      ...
      <v-card>
        <v-layout>
          <v-app-bar color="pink-darken-3" image="">
            ...
            <v-btn icon class="ml-auto" @click="createRankDialog = true">
              ...
              <v-table fixed-header>
                ...
                <tbody>
                  <tr v-for="rank in ranks" :key="rank.id" @click="handleRanksRowClick(rank.id)">
                    <td>
                      <span @click="handleShowRankMembersDialog(rank.id)"
        ...
        <div class="flex-container">
          <div class="column">
            <div cols="6">
              <h4>Unranked</h4>
              <v-table>
                <draggable class="list-group" :list="unrankedMembers" group="people" @change="updateRanking"
                  itemKey="name">
...
function getMembers(rankId: string): Member[] {
  const rank = ranks.value.find(r => r.id === rankId)
  if (rank) {
    return rank.members
  }
...
function onRankMembersDialogClose() {
  console.log("onRankMembersDialogClose")
}
