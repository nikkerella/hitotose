<template>
  <v-container>
    <v-card>
      <v-layout>
        <v-app-bar color="purple-darken-3" image="">
          ...
          <v-btn-toggle v-model="status" class="pr-3" rounded="xl" borderless>
            <v-btn value="Played" icon="mdi-progress-check" />
            <v-btn value="Playing" icon="mdi-progress-star-four-points" />
            <v-btn value="ToPlay" icon="mdi-progress-close" />
          </v-btn-toggle>
        </v-app-bar>
        ...
        <v-main>
                ...
                <v-main class="ma-1" style="min-height: 350px">
                  <div class="d-flex flex-wrap ga-5">
                    <v-card v-for="g in filteredGames(status)" :key="g.id" max-height="375" max-width="250"
                      ...
                      <v-card-actions>
                        <v-btn variant="outlined" density="comfortable" :icon="icon(g.status)"
                          @click="fetchUpdatingGame(g.id)" />
                        <v-btn variant="outlined" density="comfortable" :icon="icon(g.platform)" />
                        <v-btn variant="outlined" density="comfortable" icon>
                          <v-progress-circular :model-value="percentage(g.played_time, g.time_to_beat)">
                            <template v-slot:default>
                              <span style="font-size: 0.7rem;">{{ percentage(g.played_time, g.time_to_beat) }}</span>
                            </template>
                          </v-progress-circular>
                        </v-btn>
                        <v-btn variant="outlined" density="comfortable" icon>{{ g.rating ? g.rating : '-' }}</v-btn>
                      </v-card-actions>
                    </v-card>
                  </div>
                </v-main>
          ...
          <v-pagination v-model="currentPage" :length="totalPage" :total-visible="totalPage"></v-pagination>
        </v-main>
      </v-layout>
    </v-card>
    ...
    <!-- Update Game Dialog -->
    <div class="pa-4 text-center">
      <v-dialog v-model="updateGameDialog" max-width="600">
...
const selectOptions: SelectOptions = optionJson as SelectOptions
